...
## Описание схемы
- **Блок ПРИБОР**: моделирует цикл поломки и ремонта прибора. Прибор ломается (Тслом), переходит в состояние «сломан», ждёт свободного мастера, после ремонта возвращается в рабочее состояние.
- **Блок МАСТЕР**: моделирует занятость мастера. Ромб проверяет состояние «мастер занят/свободен»; при занятости выполняется ремонт (Трем), при свободе — ожидание.
